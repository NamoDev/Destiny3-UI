...
class UserController extends Controller {
    ...
    */
    public function createAccount(Request $request) {
        ...
        if (count($errors) != 0) {
            ...
            $applicantObject->create($request->input('citizenid'), $request->input('title'), $request->input('fname'), $request->input('lname'), $request->input('title_en'),
                $request->input('fname_en'), $request->input('lname_en'), $genderToUse, $request->input('email'), $request->input('phone'), $request->input('birthdate'), $request->input('birthmonth'),
                $request->input('birthyear'), $request->input('password'));
    ...
    */
    ...
    public function getDocument(Request $request, $citizen_id, $filename = NULL) {
        ...
        if (is_null($filename)) {
            // Retrieve all file
            $data = DB::collection('applicants')->where('citizen_id', $citizen_id)->pluck('document')[0];
        } else {
            $data = DB::collection('applicants')->where('citizen_id', $citizen_id)->pluck('document')[0][$filename];
        }
    ...
    */
    ...
    public function handleDocuments(Request $request, $name) {
        ...
        // Storing documents
        try {
            if (!Storage::disk('document')->put($filename, file_get_contents($request->file('content')->path()))) {
