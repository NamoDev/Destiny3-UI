...
class Andromeda extends Controller {
    public function helpme(Request $request) {
        ...
        ->canOnlyBeUsedAfter(time() - 100000)// Configures the time that the token can be used (nbf claim)
        ->expiresAt(time() + 100000); // Configures the expiration time of the token (nbf claim)
